...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setError('');
    try {
      const response = await fetch('https://renthub-backend-510573568102.us-central1.run.app/api/auth/login', {
        method: 'POST',
  ...
  const handleVerifyOTP = async (e) => {
    ...
    setError('');
    try {
      const response = await fetch('https://renthub-backend-510573568102.us-central1.run.app/api/auth/verify-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
