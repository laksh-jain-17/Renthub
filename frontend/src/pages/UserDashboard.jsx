  ...
  return (
    <div>
      <div
        ...
        onDrop={handleDrop}
        style={{
          border: `2px dashed ${dragOver ? GREEN : '#ccc'}`,
          borderRadius: '10px',
          padding: '20px',
          textAlign: 'center',
          cursor: 'pointer',
      ...
      >
        {preview ? (
          ...
          <>
            ...
            <p style={{ margin: 0, color: '#bbb', fontSize: '0.78rem' }}>
...
const MyListings = () => {
  ...
  const [formData, setFormData] = useState({
    title: '', category: 'camera', pricePerDay: '', description: ''
  });
  ...
  const fetchItems = async () => {
    ...
    const userId = localStorage.getItem('userId');
    try {
      const r = await fetch(`http://localhost:5000/api/items/owner/${userId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const r = await fetch('http://localhost:5000/api/items/add', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
        body: data
      });
  ...
  const handleDelete = async (itemId) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const r = await fetch(`http://localhost:5000/api/items/${itemId}`, {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'block', marginBottom: '28px' }}>
        <h3 style={{ fontSize: '2.2rem', color: '#333', fontWeight: '700' }}>
          Terms and condition
        </h3>
      ...
      {showForm && (
        <div style={{
          background: 'white', padding: '30px', borderRadius: '18px',
          marginBottom: '30px', boxShadow: '0 5px 20px rgba(0,0,0,0.07)'
        }}>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px' }}>
              ...
              <div>
                ...
                <input type="number" value={formData.pricePerDay}
            ...
            <button type="submit" style={{
              padding: '12px 30px', background: GREEN, color: 'white',
        ...
        {items.length === 0 ? (
          ...
          <div key={item._id} style={{
            ...
            <div style={{ padding: '16px' }}>
              ...
              <p style={{ color: '#aaa', fontSize: '0.8rem', marginBottom: '14px', textTransform: 'uppercase' }}>
                {item.category}
              </p>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '12px', borderTop: '1px solid #eee' }}>
                ...
                <button onClick={() => handleDelete(item._id)} style={{
                  padding: '7px 16px', background: '#fee', color: '#e53e3e',
                  border: '1px solid #fcc', borderRadius: '6px', cursor: 'pointer', fontSize: '0.82rem', fontWeight: '600'
                }}>Delete</button>
              </div>
            </div>
          </div>
...
const RentalBookings = () => {
  ...
  useEffect(() => {
    const token = localStorage.getItem('token');
    const userId = localStorage.getItem('userId');
    fetch(`http://localhost:5000/api/bookings/owner/${userId}`, {
      headers: { 'Authorization': `Bearer ${token}` }
    })
      .then(r => r.ok ? r.json() : [])
      .then(setBookings)
    ...
    fetch(`http://localhost:5000/api/bookings/owner/${userId}`, {
      ...
      .then(bookings => {
        const total = bookings.reduce((s, b) =>
          b.status === 'completed' || b.status === 'active' ? s + b.totalPrice : s, 0);
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px', marginBottom: '30px' }}>
        {[
          ...
          <div key={label} style={{
            background: bg, padding: '28px', borderRadius: '16px',
            boxShadow: '0 5px 15px rgba(0,0,0,0.07)',
            border: bg === 'white' ? '1px solid #eee' : 'none'
          }}>
...
const Payments = () => {
  ...
  return (
    <div>
      ...
      <div style={{
        display: 'inline-flex', gap: '40px', background: 'white',
        borderRadius: '16px', padding: '18px 32px',
        boxShadow: '0 5px 15px rgba(0,0,0,0.06)', marginBottom: '28px'
      }}>
        <div>
                ...
                <tr key={p._id} style={{ borderBottom: '1px solid #f5f5f5' }}>
                  ...
                  <td style={{ padding: '14px' }}>
                    <span style={{
                      ...
                      background: paid ? GREEN_LIGHT : '#ffebee',
                      color: paid ? GREEN : '#e53e3e'
                    }}>{paid ? 'Paid' : 'Cancelled'}</span>
                  </td>
...
const Profile = () => {
  ...
  const handleChangePassword = async (e) => {
    ...
    const token = localStorage.getItem('token');
    const userId = localStorage.getItem('userId');
    const r = await fetch(`http://localhost:5000/api/users/${userId}/change-password`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    ...
    <div style={{ maxWidth: '700px' }}>
      ...
      {msg.text && (
        <div style={{
          ...
          background: msg.type === 'ok' ? '#e6fffa' : '#fee',
          color: msg.type === 'ok' ? GREEN : '#c33', fontWeight: '500'
        }}>{msg.text}</div>
      )}
      ...
      <div style={{ background: 'white', borderRadius: '20px', padding: '30px', boxShadow: '0 5px 20px rgba(0,0,0,0.07)', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '22px' }}>
        ...
        <div>
          ...
          <span style={{ display: 'inline-block', marginTop: '6px', padding: '3px 12px', background: GREEN_LIGHT, color: GREEN, borderRadius: '12px', fontSize: '0.75rem', fontWeight: '700' }}>
            Buyer & Seller
          </span>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '22px' }}>
          ...
          {!editing && (
            <button onClick={() => setEditing(true)} style={{
              padding: '8px 20px', background: GREEN, color: 'white',
              border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600'
            }}>Edit</button>
          )}
        </div>
        <form onSubmit={handleUpdateProfile}>
          ...
          {editing && (
            <div style={{ display: 'flex', gap: '10px' }}>
              <button type="submit" style={{
                padding: '11px 28px', background: GREEN, color: 'white',
                border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600'
              }}>Save Changes</button>
              <button type="button" onClick={() => {
                ...
                padding: '11px 28px', background: '#f0f0f0', color: '#555',
                border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600'
              }}>Cancel</button>
            </div>
          )}
        ...
        <form onSubmit={handleChangePassword}>
          {[
            ...
            <div key={key} style={{ marginBottom: '18px' }}>
              <label style={{ display: 'block', marginBottom: '6px', color: '#666', fontSize: '0.85rem', fontWeight: '500' }}>{label}</label>
              <input type="password" value={pwData[key]}
...
const EmptyState = ({ icon, title, sub }) => (
  <div style={{
    ...
    textAlign: 'center', color: '#999', boxShadow: '0 5px 15px rgba(0,0,0,0.05)'
  }}>
